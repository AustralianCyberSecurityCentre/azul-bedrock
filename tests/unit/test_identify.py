...
class TestBasic(unittest.TestCase):
    def test_import1(self):
        ...
        magic, mime, file_format, file_extension = identify.from_file(path)
    ...
    def test_import2(self):
        ...
        magic, mime, file_format, file_extension = identify.from_buffer(file_bytes)
        self.assertEqual(
            magic,
...
with open(loc, "r") as f:
    try:
        ...
        for tc in test_yaml.identify_tests:
            found_test_cases.append((tc.sha256, tc.file_format, tc.file_extension, tc.magic, tc.mime))
    except Exception:
...
@pytest.mark.parametrize(
    "sha256,file_format,file_extension,magic,mime",
    found_test_cases,
)
def test_file_formats(sha256: str, file_format: str, file_extension: str, magic: str, mime: str):
